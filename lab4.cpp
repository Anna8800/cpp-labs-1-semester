...
int _tmain()
{
	...
	//Объявите ссылку на тип double. Посредством ссылки измените значение
	{
		...
		//внешняя ссылка проинициализирована в другом файле (other.cpp) при определении
	...
	}
	{
		...
		int ar[N][M] = { {34, 78, 18, 90, 26}, {12, 87, 38, 31, 92}, {9, 23, 83, 16, 33}, {15, 56, 52, 45, 97} };
		int res = findMin(ar, N);
		std::cout << "Min element in arr: " << res << std::endl;

		//2. динамический двухмерный массив
		int dynrow, dyncol;
		srand(static_cast<int>(time(0)));
		dynrow = rand() % 10; //ограничим рандомные числа до 10, чтобы не получить слишком большую матрицу
		dyncol = rand() % 10;
